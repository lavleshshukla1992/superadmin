    ...
    <div class="main-content-inner">
        <div class="row">
            ...
            <div class="col-12 mt-5">
                <div class="card">
                    <div class="card-body">
                        {!! Form::model($notice,['route'=>['notice.update',$notice],'method'=>'PATCH', 'enctype' => 'multipart/form-data']) !!}
                            <div class="row">
                                <div class="col-md-6">
                                            ...
                                            <div class="checkbox-columns" id="gender">
                                                @foreach(['0' => 'All Gender','male' => 'Male', 'female'=>'Female', 'other'=>'Other'] as $key=>$result)
                                                    <div class="checkbox-column">
                                                        <input type="checkbox" name="gender[]" id="{{ $key }}" value="{{ $key }}" {{ (in_array($key, explode(",", $notice->gender))) ? 'checked' : '' }}>
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <div class="checkbox-columns" id="social_category">
                                                @foreach(['0' => 'All Social Category','Sc' => 'SC', 'ST'=>'ST', 'OBC'=>'OBC', 'General'=>'General', 'EWS'=>'EWS', 'Open'=>'Open'] as $key=>$result)
                                                    <div class="checkbox-column">
                                                        <input type="checkbox" name="social_category[]" id="{{ $key }}" value="{{ $key }}" {{ (in_array($key, explode(",", $notice->social_category))) ? 'checked' : '' }}>
                                                        <label for="{{ $key }}">{{ $result }}</label>
                                                    </div>
                                                @endforeach
                                            </div>
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <div class="checkbox-columns" id="educational_qualification">
                                                @foreach(['0' => 'All Educational Qualification','primary_education' => 'Primary Education', 'high_school' => 'High School', 'Intermediate' => 'Intermediate', 'Undergraduate' => 'Undergraduate', 'Postgraduate' => 'Postgraduate'] as $key=>$result)
                                                    <div class="checkbox-column">
                                                        <input type="checkbox" name="educational_qualification[]" id="{{ $key }}" value="{{ $key }}" {{ (in_array($key, explode(",", $notice->educational_qualification))) ? 'checked' : '' }}>
                                                        <label for="{{ $key }}">{{ $result }}</label>
                                                    </div>
                                                @endforeach
                                            </div>
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <div class="checkbox-columns" id="type_of_vending">
                                                @foreach($vending as $key=>$result)
                                                    <div class="checkbox-column">
                                                        <input type="checkbox" name="type_of_vending[]" id="{{ $key }}" value="{{ $key }}" {{ (in_array($key, explode(",", $notice->type_of_vending))) ? 'checked' : '' }}>
                                                        <label for="{{ $key }}">{{ $result }}</label>
                                                    </div>
                                                @endforeach
                                            </div>
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <div class="checkbox-columns" id="type_of_marketplace">
                                                @foreach($marketplace as $key=>$result)
                                                    <div class="checkbox-column">
                                                        <input type="checkbox" name="type_of_marketplace[]" id="type_of_marketplace{{ $key }}" value="{{ $key }}" {{ (in_array($key, explode(",", $notice->type_of_marketplace))) ? 'checked' : '' }}>
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <div class="checkbox-columns" id="state_id">
                                                @foreach($state as $key=>$result)
                                                    <div class="checkbox-column">
                                                        <input type="checkbox" name="state_id[]" id="state_id{{ $key }}" value="{{ $key }}" {{ (in_array($key, explode(",", $notice->state_id))) ? 'checked' : '' }}>
        ...
        if (selecteddistricts.length > 1) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $('#municipality_id').empty();
                    var municipality = "<?= $notice->municipality_id ?>";
            ...
            if (selectedStates.length > 1 || selectedStates.length < 1 ) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        $('#district_id').empty();
                        var district = "<?= $notice->district_id ?>";
