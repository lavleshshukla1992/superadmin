...
class MembershipDashBoard
{
    ...
    public static function demographySpecific($fromDate, $toDate)
    {
        ...
        $demographySpecific = $demographySpecific
    ...
    public static function genderSpecific($fromDate, $toDate)
    {
        ...
        if (!empty($requestData['state_id'])) {
            $state_id = explode(',', $requestData['state_id']);
            $genderSpecific->whereIn('current_state', $state_id);
        }
        if (!empty($requestData['district_id'])) {
            $district_id = explode(',', $requestData['district_id']);
            $genderSpecific->whereIn('current_district', $district_id);
        }
        if (!empty($requestData['municipality_id'])) {
            $municipality_id = explode(',', $requestData['municipality_id']);
            $genderSpecific->whereIn('municipality_panchayat_current', $municipality_id);
        }
    ...
    {
        $socialCategoryWiseCount = VendorDetail::query();
        ...
        $results = [
            /*'new_user' => $socialCategoryWiseCount->whereBetween('created_at', [$fromDate, $toDate])->count('id'),
            'total_user' => $socialCategoryWiseCount->count('id'),
            'left_user' => $socialCategoryWiseCount->onlyTrashed()->whereBetween('created_at', [$fromDate, $toDate])->count('id'),*/

            //'total_user' => $socialCategoryWiseCount->whereBetween('created_at', [$fromDate, $toDate])->count('id'),
            'total_user' => $socialCategoryWiseCount->whereDate('created_at','>=',$fromDate)->whereDate('created_at','<=',$toDate)->count('id'),
            'new_user' => $socialCategoryWiseCount->count('id') + $socialCategoryWiseCount->onlyTrashed()->whereBetween('created_at', [$fromDate, $toDate])->count('id'),
            'left_user' => $socialCategoryWiseCount->onlyTrashed()->whereBetween('created_at', [$fromDate, $toDate])->count('id'),
    ...
    public static function memberAgeSpecific($fromDate, $toDate)
    {
        ...
        if (!empty($requestData['state_id'])) {
            $state_id = explode(',', $requestData['state_id']);
            $ageRangesData->whereIn('current_state', $state_id);
        }
        if (!empty($requestData['district_id'])) {
            $district_id = explode(',', $requestData['district_id']);
            $ageRangesData->whereIn('current_district', $district_id);
        }
        if (!empty($requestData['municipality_id'])) {
            $municipality_id = explode(',', $requestData['municipality_id']);
            $ageRangesData->whereIn('municipality_panchayat_current', $municipality_id);
        }

        $ageRangesData = $ageRangesData
    ...
    public static function memberEducationalSepecific($fromDate, $toDate)
    {
        ...
        if (!empty($requestData['state_id'])) {
            $state_id = explode(',', $requestData['state_id']);
            $educationalQualification->whereIn('current_state', $state_id);
        }
        if (!empty($requestData['district_id'])) {
            $district_id = explode(',', $requestData['district_id']);
            $educationalQualification->whereIn('current_district', $district_id);
        }
        if (!empty($requestData['municipality_id'])) {
            $municipality_id = explode(',', $requestData['municipality_id']);
            $educationalQualification->whereIn('municipality_panchayat_current', $municipality_id);
        }
    ...
    public static function vendingTypeSpecific($fromDate, $toDate)
    {
        ...
        if (!empty($requestData['state_id'])) {
            $state_id = explode(',', $requestData['state_id']);
            $vendingTypeSpecific->whereIn('current_state', $state_id);
        }
        if (!empty($requestData['district_id'])) {
            $district_id = explode(',', $requestData['district_id']);
            $vendingTypeSpecific->whereIn('current_district', $district_id);
        }
        if (!empty($requestData['municipality_id'])) {
            $municipality_id = explode(',', $requestData['municipality_id']);
            $vendingTypeSpecific->whereIn('municipality_panchayat_current', $municipality_id);
        }

        $vendingTypeSpecific = $vendingTypeSpecific
    ...
    public static function marketplaceTypeSpecific($fromDate, $toDate)
    {
        $marketPlaceTypeSpecific = VendorDetail::whereBetween('created_at', [$fromDate, $toDate])
            ...
            if (!empty($requestData['state_id'])) {
                $state_id = explode(',', $requestData['state_id']);
                $marketPlaceTypeSpecific->whereIn('current_state', $state_id);
            }
            if (!empty($requestData['district_id'])) {
                $district_id = explode(',', $requestData['district_id']);
                $marketPlaceTypeSpecific->whereIn('current_district', $district_id);
            }
            if (!empty($requestData['municipality_id'])) {
                $municipality_id = explode(',', $requestData['municipality_id']);
                $marketPlaceTypeSpecific->whereIn('municipality_panchayat_current', $municipality_id);
            }
    
            $marketPlaceTypeSpecific = $marketPlaceTypeSpecific
    ...
    public static function maritalStatusSpecific($fromDate, $toDate)
    {
        ...
        if (!empty($requestData['state_id'])) {
            $state_id = explode(',', $requestData['state_id']);
            $maritalStatusWise->whereIn('current_state', $state_id);
        }
        if (!empty($requestData['district_id'])) {
            $district_id = explode(',', $requestData['district_id']);
            $maritalStatusWise->whereIn('current_district', $district_id);
        }
        if (!empty($requestData['municipality_id'])) {
            $municipality_id = explode(',', $requestData['municipality_id']);
            $maritalStatusWise->whereIn('municipality_panchayat_current', $municipality_id);
        }

        $maritalStatusWise = $maritalStatusWise
    ...
    public static function dateWiseData($fromDate, $toDate)
    {
        ...
        $dateWiseJoinedUserData = VendorDetail::withTrashed()->select(DB::raw('DATE(created_at) as date'), DB::raw('COUNT(*) as count'))
        ...
        if (!empty($requestData['state_id'])) {
            $state_id = explode(',', $requestData['state_id']);
            $dateWiseJoinedUserData->whereIn('current_state', $state_id);
        }
        if (!empty($requestData['district_id'])) {
            $district_id = explode(',', $requestData['district_id']);
            $dateWiseJoinedUserData->whereIn('current_district', $district_id);
        }
        if (!empty($requestData['municipality_id'])) {
            $municipality_id = explode(',', $requestData['municipality_id']);
            $dateWiseJoinedUserData->whereIn('municipality_panchayat_current', $municipality_id);
        }

        $dateWiseJoinedUserData = $dateWiseJoinedUserData
            ->groupBy('created_at')
        ...
        $dateWiseLeftUserData = VendorDetail::onlyTrashed()->select(DB::raw('DATE(created_at) as date'), DB::raw('COUNT(*) as count'))
            ...
            if (!empty($requestData['state_id'])) {
                $state_id = explode(',', $requestData['state_id']);
                $dateWiseLeftUserData->whereIn('current_state', $state_id);
            }
            if (!empty($requestData['district_id'])) {
                $district_id = explode(',', $requestData['district_id']);
                $dateWiseLeftUserData->whereIn('current_district', $district_id);
            }
            if (!empty($requestData['municipality_id'])) {
                $municipality_id = explode(',', $requestData['municipality_id']);
                $dateWiseLeftUserData->whereIn('municipality_panchayat_current', $municipality_id);
            }

            $dateWiseLeftUserData = $dateWiseLeftUserData
            ->groupBy('created_at')
            ->orderBy('created_at')->toBase()
            ->get();
        ...
        foreach ($result as $key => $value) {
            foreach ($dateWiseJoinedUserData as  $joinedData) {//print_r($key); echo "<br>";
...
//die;
    ...
    public static function adminDateWise($fromDate, $toDate)
    {
        ...
        //echo "<pre>"; print_r($dateWiseJoinedUserData);die;
    ...
    public static function adminMemberAgeSpecific($fromDate, $toDate)
    {
        ...
        return [
            ...
            'leftvalues' => json_encode($leftvalues)
        ];
        //return json_encode($agedata);
    }
